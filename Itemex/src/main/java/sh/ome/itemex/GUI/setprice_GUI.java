...
import static sh.ome.itemex.commands.commands.constructItem;
import static sh.ome.itemex.commands.commands.get_json_from_meta;

public class setprice_GUI {

    public static void generateGUI(Player p, String title, String itemname, int amount, boolean market_or_limit) {

        double price;

        if(market_or_limit) {
            if(title.contains("BUY"))
                price = Itemex.getPlugin().mtop.get( get_json_from_meta(itemname) ).get_top_sellorder_prices()[0];
            else
                price = Itemex.getPlugin().mtop.get( get_json_from_meta(itemname) ).get_top_buyorder_prices()[0];
        }
        else {
            if(title.contains("BUY"))
                price = Itemex.getPlugin().mtop.get( get_json_from_meta(itemname) ).get_top_buyorder_prices()[0];
        ...
        confirm_lore.add(ChatColor.GRAY + "Amount: " + ChatColor.GOLD + amount);
        if(market_or_limit)
            confirm_lore.add(ChatColor.GRAY + "Total: ~" + ChatColor.DARK_GREEN + total);
        else
            confirm_lore.add(ChatColor.GRAY + "Total: " + ChatColor.DARK_GREEN + total);
        ...
        // ITEMID
        ItemStack item = constructItem( get_json_from_meta(itemname), amount);
        ...
        item_lore.add(ChatColor.DARK_GRAY + "Price: " + price);
